...
const slack_client = new SlackBot({token: SLACK_TOKEN, name: 'slack-dumper'});
...
discord_client.on('message', function(message) {
	...
	{
		debug("Discord --> " + message.author.username + ": " + message.content);
		slack_client.postMessageToChannel(SLACK_CHANNEL, message.author.username + ": " + message.content);
	}
});
